...
class ParkedState implements CarState {
    public void changeGear(Car car) {
        // Car.Car can only change gear when it's not parked
...
class DriveState implements CarState {
    public void changeGear(Car car) {
        // Car.Car can change gear to higher gear when driving at certain speed
        if (car.getSpeed() < 20) {
...
class ReverseState implements CarState {
    public void changeGear(Car car) {
        // Car.Car can only change to reverse gear when speed is 0
        if (car.getSpeed() > 0) {
...
class Car {
    ...
    public Car(int maxGear) {
        ...
        this.state = new ParkedState(); // By Default every car is in Parked State
    }
...
public class Main {
    public static void main(String[] args) {
        ...
        // Car.Car cannot shift to higher gear when already in top gear
